...
11. **НЕЛЬЗЯ ставить `overflow: hidden` на контейнер модалки** — обрезает border, shadow и скруглённые углы. `overflow: hidden` только на внутренний scroll-wrapper.

### Git и деплой
12. **НЕЛЬЗЯ пушить в `main` или `mobile-dev` напрямую.** Только в `mobile-dev3.0`.
13. **НЕЛЬЗЯ пушить файл без чтения его текущего SHA** (иначе конфликт и потеря данных).
14. **НЕЛЬЗЯ угадывать структуру существующего файла** — всегда читать перед редактированием.
15. **НЕЛЬЗЯ делать несколько коммитов когда можно один** — использовать `push_files` для батч-пуша.

### БД и API
16. **НЕЛЬЗЯ делать прямые SQL-миграции без записи в ROADMAP** (что добавлено/изменено в таблице).
17. **НЕЛЬЗЯ хранить JWT в AsyncStorage** — только в `SecureStore` через `storage.js`.
18. **НЕЛЬЗЯ делать DELETE без подтверждения** (`AlertModal.js`).

### Документация
19. **НЕЛЬЗЯ завершать этап без обновления** `DEVLOG_v3.md`, `ROADMAP_v3.md`, `QA_v3.md`.
20. **НЕЛЬЗЯ завершать сессию без обновления** `AI_CONTEXT.md` (этот файл).
...
| `Modal.js` | Модальное окно. Pressable-backdrop, ScrollView внутри. **Без `overflow:hidden` на контейнере** — только на scroll-wrapper |
| `Card.js` | Карточка |
...
| `CalendarScreen.js` | **Календарь** — недельный/месячный вид, DayPanel (bottom sheet), индикаторы по дням, ДР и личные события |
...
### PanGestureHandler — свайп без конфликта со скроллом
```js
// Только горизонтальный свайп — не мешает вертикальному скроллу вложенных компонентов
<PanGestureHandler
  activeOffsetX={[-15, 15]}   // срабатывает только при горизонтальном движении
  failOffsetY={[-10, 10]}     // отменяется если вертикальное движение первично
  onGestureEvent={onSwipe}
>
```

### Modal.js — структура без обрезки углов
```jsx
// modal: без overflow:hidden → border/shadow/borderRadius видны целиком
// scrollWrapper: overflow:hidden + borderBottomRadius → контент не вылезает
...
// scrollWrapper: { borderBottomLeftRadius: 18, borderBottomRightRadius: 18, overflow: 'hidden' }
```
...
- ✅ **Этап 4** — Календарь (недельный/месячный вид, DayPanel, индикаторы, ДР/события, свайп недели)

### Следующий шаг
- **Этап 5** — Уведомления: экран настройки (утро/вечер/ДР), напоминание за 10 мин до задачи с временем, убрать тестовую кнопку из профиля
